...
public class BigramFitness extends FitnessFunction {
    ...
    public BigramFitness(boolean isAnalysingRotors) {
        // Bigrams
        this.bigrams = new float[826];
        Arrays.fill(this.bigrams, (float)Math.log10(epsilon));
        String filePath = isAnalysingRotors ? "/data/bigrams" : "/data/plug_entropy/bigrams";
        try (final InputStream is = BigramFitness.class.getResourceAsStream(filePath);
